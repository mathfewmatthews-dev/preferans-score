...
test("Полвиста: контракт записывается без ввода взяток", async ({ page }) => {
  ...
  await expect(page.locator('[data-role="2"][type="hidden"]')).toHaveValue("Пас");
  await expect(page.locator('[data-role-button="2"][data-role="Пас"]')).toHaveClass(/active/);
  await page.locator('[data-role-button="2"][data-role="Вист"]').click();
  await expect(page.locator('[data-role="1"][type="hidden"]')).toHaveValue("Пас");
  await page.locator('[data-role-button="1"][data-role="Полвиста"]').click();
  await expect(page.locator('[data-role="2"][type="hidden"]')).toHaveValue("Пас");
